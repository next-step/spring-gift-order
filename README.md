...
- [x] `feat`: 주문(Order) API DTO(Request, Response) 추가
- [x] `feat`: 주문 생성 서비스 로직 구현 (재고 차감 포함)
- [x] `refactor`: 주문 시 위시리스트 상품 제거 로직 추가
- [x] `feat`: 카카오톡 나에게 메시지 보내기 기능 클라이언트 추가
- [x] `feat`: 주문 완료 후 카카오톡 메시지 발송 기능 연동
- [x] `feat`: 주문 생성 API 컨트롤러 구현
- [x] `test`: 주문 생성 통합 테스트 작성

## 구현할 기능 목록 (3단계 - 배포하기)

- [x] `docs`: 3단계 배포 기능 목록 추가
- [x] `chore`: 배포 스크립트 작성
- [x] `feat`: CORS 설정 추가하여 클라이언트 연동 문제 해결
- [ ] `test`: CORS 설정 테스트 코드 작성
