...
public abstract class AbstractOrderTest extends AbstractControllerTest {
    ...
    protected Map<UserRole, OptionResponse> testOptions;
    ...
    public void setUp(RestDocumentationContextProvider provider) {
        ...
        Map.of(
            ...
            var optionRequest1 = new OptionCreateRequest("Init Option", 10L);
            var optionRequest2 = new OptionCreateRequest(role.name() + " Option", 100L);
            var productRequest = new ProductCreateRequest(role.name() + " Prod", 1000L, "www.example.com/image.jpg",
                    List.of(optionRequest1));
            var product = restAssuredUtils.createProduct(productRequest, token);
            this.testProducts.put(role, product);
            this.testOptions.put(role, restAssuredUtils.createOptionToProduct(optionRequest2, product.id(), token));
